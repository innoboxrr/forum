...
<div id="forum" class="discussion">
	...
	<div id="forum_header" style="background-color:{{ $discussion->color }}">
		<div class="container">
			<a class="back_btn" href="/{{ Config::get('forum.routes.home') }}"><i class="forum-back"></i></a>
			<h1>{{ $discussion->title }}</h1><span class="forum_head_details"> @lang('forum::messages.discussion.head_details')<a class="forum_cat" href="/{{ Config::get('forum.routes.home') }}/{{ Config::get('forum.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		</div>
	</div>
	...
	@if(config('forum.errors'))
		@if(Session::has('forum_alert'))
			<div class="forum-alert alert alert-{{ Session::get('forum_alert_type') }}">
				<div class="container">
					<strong><i class="forum-alert-{{ Session::get('forum_alert_type') }}"></i> {{ Config::get('forum.alert_messages.' . Session::get('forum_alert_type')) }}</strong>
            ...
            @else
                <div class="col-md-3 left-column">
                    ...
                    <div class="forum_sidebar">
                        ...
                        <a href="/{{ Config::get('forum.routes.home') }}"><i class="forum-bubble"></i> @lang('forum::messages.discussion.all')</a>
                        <ul class="nav nav-pills nav-stacked">
                            <?php $categories = Innoboxrr\Forum\Models\Models::category()->all(); ?>
                            @foreach($categories as $category)
                                <li><a href="/{{ Config::get('forum.routes.home') }}/{{ Config::get('forum.routes.category') }}/{{ $category->slug }}"><div class="forum-box" style="background-color:{{ $category->color }}"></div> {{ $category->name }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                    <!-- END SIDEBAR -->
            ...
            @endif
		                		...
		                		<span class="forum_posts">
			                		...
			                		<div class="forum_avatar">
					        			@if(Config::get('forum.user.avatar_image_database_field'))
					        				...
					        				@else
					        					<img src="{{ Config::get('forum.user.relative_url_to_image_assets') . $post->user->{$db_field}  }}">
					        				@endif

					        			@else
					        				<span class="forum_avatar_circle" style="background-color:#<?= \Innoboxrr\Forum\Helpers\ForumHelper::stringToColorCode($post->user->{Config::get('forum.user.database_field_with_user_name')}) ?>">
					        		...
					        		<div class="forum_middle">
					        			<span class="forum_middle_details"><a href="{{ \Innoboxrr\Forum\Helpers\ForumHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('forum.user.database_field_with_user_name')}) }}</a> <span class="ago forum_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
	            	...
	            	<div id="new_response">
	            		...
	            		<div class="forum_avatar">
		        			@if(Config::get('forum.user.avatar_image_database_field'))
		        				...
		        				@else
		        					<img src="{{ Config::get('forum.user.relative_url_to_image_assets') . Auth::user()->{$db_field}  }}">
		        				@endif

		        			@else
		        				<span class="forum_avatar_circle" style="background-color:#<?= \Innoboxrr\Forum\Helpers\ForumHelper::stringToColorCode(Auth::user()->{Config::get('forum.user.database_field_with_user_name')}) ?>">
						...
						<div id="discussion_response_email">
							...
							@if(Config::get('forum.email.enabled'))
								<div id="notify_email">
									<img src="{{ url('/vendor/innoboxrr/forum/assets/images/email.gif') }}" class="forum_email_loader">
				...
				@else
					...
					<div id="login_or_register">
						<p>
                            @lang('forum::messages.auth', ['home' => Config::get('forum.routes.home')])
                        </p>
					</div>
    ...
    @if(Config::get('forum.sidebar_in_discussion_view'))
        <div id="new_discussion_in_discussion_view">
            ...
            <form id="forum_form_editor_in_discussion_view" action="/{{ Config::get('forum.routes.home') }}/{{ Config::get('forum.routes.discussion') }}" method="POST">
                <div class="row">
                    <div class="col-md-7">
                        <!-- TITLE -->
                ...
                <div id="new_discussion_footer">
                    ...
                    <button id="submit_discussion" class="btn btn-success pull-right"><i class="forum-new"></i> Create {{ Config::get('forum.titles.discussion') }}</button>
                    <a href="/{{ Config::get('forum.routes.home') }}" class="btn btn-default pull-right" id="cancel_discussion">Cancel</a>
