...
namespace Innoboxrr\Forum\Tests\Package;

use Illuminate\Support\Facades\Route;
use Innoboxrr\Forum\Tests\TestCase;
...
final class PackageBootsTest extends TestCase
{
    ...
    {
        foreach (self::declaredProviders() as $provider) {
    ...
    /**
     * Sobre SQLite en memoria: no prueba lo que es propio de MySQL, pero si
     * que cada migracion es PHP valido y que el orden funciona. No se deshacen:
     * SQLite no puede borrar una columna con clave foranea, y un fallo propio
     * del motor de pruebas no dice nada del paquete.
     */
    public function test_las_migraciones_corren(): void
    {
        // migrator->path() y no loadMigrationsFrom(): Testbench deshace al
        // terminar lo que carga con este ultimo, y es justo el rollback que
        // SQLite no sabe hacer.
    ...
    private static function classes(): array
    {
        ...
        foreach (self::composer()['autoload']['psr-4'] ?? [] as $prefix => $paths) {
    ...
    private static function fromSuggestedPackage(string $message): bool
    {
        ...
        foreach (array_keys(self::composer()['suggest'] ?? []) as $package) {
